...
const Header = () => {
  ...
  return (
    <>
      {/*
        ── One sticky wrapper for BOTH the top bar and the category nav ──
        This way they scroll together as one block and never separate.
      */}
      <header className="sticky top-0 z-[100] w-full bg-white/80 backdrop-blur-lg border-b border-gray-100">
        ...
        <div className="max-w-screen-2xl mx-auto flex justify-between items-center py-3 md:py-4 px-3 md:px-6">
          ...
          <div className="flex-1 flex gap-1 md:gap-4 items-center justify-end">
            ...
            <button
              ...
              className="language-toggle-btn-mobile md:language-toggle-btn relative"
            ...
            >
              ...
              <span className="hidden md:flex items-center gap-2">
                ...
                <span className="lang-text">
                  <span className={`lang-label ${i18n.language === "en" ? "translate-y-0" : "-translate-y-full"}`}>AR</span>
                  <span className={`lang-label ${i18n.language === "en" ? "translate-y-full" : "translate-y-0"}`}>EN</span>
                </span>
              </span>
              {isLangAnimating && (
            ...
            {/* Search */}
            <Link to="/search" className="p-1.5 md:p-2 hover:text-gray-500 transition-colors shrink-0" aria-label="Search">
              <HiOutlineMagnifyingGlass className="text-xl md:text-2xl" />
            </Link>

            {/* Auth */}
            {loginStatus ? (
              <button onClick={handleLogout} className="p-1.5 md:p-2 hover:text-red-500 transition-colors shrink-0" aria-label="Logout">
                <HiOutlineArrowRightOnRectangle className="text-xl md:text-2xl" />
              </button>
            ) : (
              <Link to="/login" className="p-1.5 md:p-2 hover:text-gray-500 transition-colors shrink-0" aria-label="Login">
                <HiOutlineUser className="text-xl md:text-2xl" />
              </Link>
            )}

            {/* Cart */}
            <Link to="/cart" className="p-1.5 md:p-2 relative hover:scale-110 transition-transform shrink-0" aria-label="Cart">
