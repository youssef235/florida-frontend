...
───────────────────────────────────────────── */
const ProductDetailsSection = ({ description, name, categories }: {
  description?: string;
  name?: string;
  categories?: any[];
}) => {
  ...
  const highlights = [
    { icon: <HiOutlineTag className="w-4 h-4" />, label: categories?.[0]?.name || "Collection" },
  ];
  ...
  return (
    <div className="mt-8 pt-8 border-t border-gray-100">
      ...
      <p className="text-[10px] font-bold uppercase tracking-[0.3em] text-gray-400 mb-4">
        {t("product.product_details") || "Product Details"}
      </p>
      ...
      <div className="flex flex-wrap gap-2">
        {highlights.map((h, i) => (
          <span
            key={i}
            className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-gray-50 border border-gray-100 text-[11px] font-medium text-gray-600"
          >
            {h.icon}
            {h.label}
          </span>
...
const SingleProduct = () => {
  ...
  useEffect(() => {
    const fetchSingleProduct = async () => {
      try {
        ...
        if (product.priceTags?.length > 0) setSelectedPriceTag(product.priceTags[0]);
      } catch (error) {
        console.error(error);
  ...
  const nextSlide = useCallback(() => {
    ...
    setCurrentIndex((prev) => (prev < singleProduct.images.length - 1 ? prev + 1 : 0));
  }, [singleProduct?.images?.length]);

  const prevSlide = useCallback(() => {
    if (!singleProduct?.images?.length) return;
    setCurrentIndex((prev) => (prev > 0 ? prev - 1 : singleProduct.images.length - 1));
  }, [singleProduct?.images?.length]);

  const goToSlide = (index: number) => setCurrentIndex(index);

  const handleTouchStart = (e: React.TouchEvent) => { setTouchStart(e.targetTouches[0].clientX); setIsDragging(true); };
  const handleTouchMove = (e: React.TouchEvent) => { if (!isDragging) return; setTouchEnd(e.targetTouches[0].clientX); dragX.set(-(touchStart - e.targetTouches[0].clientX)); };
  const handleTouchEnd = () => {
    setIsDragging(false); dragX.set(0);
    if (!touchStart || !touchEnd) return;
    const distance = touchStart - touchEnd;
    if (distance > 50) nextSlide(); else if (distance < -50) prevSlide();
    setTouchStart(0); setTouchEnd(0);
  };
  const handleMouseDown = (e: React.MouseEvent) => { setTouchStart(e.clientX); setIsDragging(true); };
  ...
  const onDragEnd = () => { const x = dragX.get(); if (x <= -50) nextSlide(); else if (x >= 50) prevSlide(); };

  const handleAddToCart = async () => {
    if (singleProduct.sizes?.length > 0 && !selectedSize) { toast.error(t("product.select_size")); return; }
    if (singleProduct.colors?.length > 0 && !selectedColor) { toast.error(t("product.select_color")); return; }
  ...
  const toggleWishlist = async () => {
    if (!params.id) return;
    if (isWishlisted) { await dispatch(removeFromWishlist(params.id)); toast.success(t("product.removed_from_wishlist")); }
    else { await dispatch(addToWishlist(params.id)); toast.success(t("product.added_to_wishlist")); }
  ...
  if (!singleProduct) return (
    <div className="h-screen w-full flex flex-col items-center justify-center gap-4">
      ...
      <span className="text-gray-400 uppercase tracking-widest text-xs">{t("product.loading")}</span>
    </div>
  );
    ...
    <div className="bg-white min-h-screen pb-32 lg:pb-0">
      <div className="max-w-[1920px] mx-auto flex flex-col lg:flex-row">
        ...
        <div className="relative w-full lg:w-[60%] h-[50vh] sm:h-[60vh] md:h-[70vh] lg:h-screen overflow-hidden bg-[#f9f9f9]">
          ...
          >
            <AnimatePresence mode="popLayout" initial={false}>
              <motion.img
                ...
                animate={{ opacity: imageStatus === "loaded" ? 1 : 0.5, scale: 1 }}
          ...
          {/* Desktop arrows */}
          <button onClick={prevSlide} className="hidden lg:flex absolute left-4 top-1/2 -translate-y-1/2 p-3 bg-white/80 backdrop-blur-md rounded-full hover:bg-white transition-all shadow-lg z-10 hover:scale-110" aria-label="Previous image">
            <HiOutlineChevronLeft className="w-5 h-5" />
          </button>
          <button onClick={nextSlide} className="hidden lg:flex absolute right-4 top-1/2 -translate-y-1/2 p-3 bg-white/80 backdrop-blur-md rounded-full hover:bg-white transition-all shadow-lg z-10 hover:scale-110" aria-label="Next image">
          ...
          {/* Zoom */}
          <button onClick={() => setShowLightbox(true)} className="absolute top-4 left-4 lg:top-6 lg:left-6 p-2.5 bg-white/80 backdrop-blur-sm rounded-full hover:bg-white transition-all shadow-lg z-10 hover:scale-110" aria-label="Zoom image">
          ...
          <div className="absolute bottom-4 w-full flex justify-center gap-2 z-10 lg:hidden">
            {singleProduct.images?.map((_: any, i: number) => (
              <button key={i} onClick={() => goToSlide(i)} className={`h-1.5 rounded-full transition-all duration-300 ${i === currentIndex ? "w-8 bg-black" : "w-1.5 bg-black/30"}`} aria-label={`Go to image ${i + 1}`} />
            ))}
          </div>

          {/* Mobile back */}
          <button onClick={() => window.history.back()} className="lg:hidden absolute top-4 left-4 p-2.5 bg-white/80 backdrop-blur-sm rounded-full shadow-sm z-10" aria-label="Go back">
            <HiOutlineChevronLeft className="w-5 h-5" />
          </button>
          ...
          <button
            onClick={() => {
              if (navigator.share) { navigator.share({ title: singleProduct.name, url: window.location.href }); }
              else { navigator.clipboard.writeText(window.location.href); toast.success(t("product.link_copied")); }
            }}
        ...
        <div className="w-full lg:w-[40%] px-4 sm:px-6 py-6 lg:py-12 lg:px-12 xl:px-16 lg:sticky lg:top-0 lg:h-screen lg:overflow-y-auto bg-white">
          ...
          <div className="flex justify-between items-start mb-2">
            ...
            <button onClick={toggleWishlist} className="relative p-2 rounded-full hover:bg-gray-100 transition-all group" aria-label={isWishlisted ? "Remove from wishlist" : "Add to wishlist"}>
              <AnimatePresence mode="wait">
                {isWishlisted ? (
                  <motion.div key="filled" initial={{ scale: 0 }} animate={{ scale: 1 }} exit={{ scale: 0 }} transition={{ type: "spring", stiffness: 500, damping: 30 }}>
                    <HiHeart className="w-6 h-6 text-red-500" />
                  </motion.div>
                ) : (
                  <motion.div key="outline" initial={{ scale: 0 }} animate={{ scale: 1 }} exit={{ scale: 0 }} transition={{ type: "spring", stiffness: 500, damping: 30 }}>
          ...
          {hasSizes && (
            <div className="mb-6">
              <div className="flex justify-between items-center text-[11px] font-bold uppercase tracking-wider mb-3 text-gray-500">
                ...
                {selectedSize && <span className="text-black font-bold bg-gray-100 px-2 py-0.5 rounded">{selectedSize}</span>}
              </div>
              <div className="flex flex-wrap gap-2">
          ...
          {hasColors && (
            <div className="mb-6">
              <div className="flex justify-between items-center text-[11px] font-bold uppercase tracking-wider mb-3 text-gray-500">
                ...
                {selectedColor && <span className="text-black font-medium">{selectedColor.name}</span>}
              </div>
              <div className="flex flex-wrap gap-3">
                {singleProduct.colors.map((color: any) => (
                  <button key={color.hex} onClick={() => setSelectedColor(color)} title={color.name} className="relative group">
                    <div
                    ...
                    />
                    {selectedColor?.hex === color.hex && (
                      <motion.div layoutId="colorRing" className="absolute -inset-1 rounded-full border-2 border-black" transition={{ type: "spring", stiffness: 400, damping: 30 }} />
                    )}
                  </button>
                ))}
              </div>
          ...
          {/* ── Product Details Section (always visible, no dropdown) ── */}
      ...
      <div className="lg:hidden fixed bottom-0 left-0 right-0 px-4 py-3 bg-white/95 backdrop-blur-xl border-t border-gray-100 z-[99] flex gap-3 items-center shadow-[0_-4px_20px_rgba(0,0,0,0.08)]">
        <div className="flex flex-col justify-center min-w-[90px]">
          <span className="text-[10px] text-gray-400 uppercase font-bold tracking-tight">{t("product.total_price")}</span>
      ...
      <AnimatePresence>
        {showLightbox && (
          ...
          >
            <button onClick={() => setShowLightbox(false)} className="absolute top-4 right-4 p-3 bg-white/10 rounded-full text-white hover:bg-white/20 transition-all z-10">
              <HiOutlineXMark className="w-6 h-6" />
            </button>
            <button onClick={(e) => { e.stopPropagation(); prevSlide(); }} className="absolute left-4 top-1/2 -translate-y-1/2 p-3 bg-white/10 rounded-full text-white hover:bg-white/20 transition-all">
              <HiOutlineChevronLeft className="w-6 h-6" />
            </button>
            <button onClick={(e) => { e.stopPropagation(); nextSlide(); }} className="absolute right-4 top-1/2 -translate-y-1/2 p-3 bg-white/10 rounded-full text-white hover:bg-white/20 transition-all">
              <HiOutlineChevronRight className="w-6 h-6" />
            </button>
            <motion.img
              key={currentIndex} src={imageSrc}
            ...
            <div className="absolute bottom-6 left-1/2 -translate-x-1/2 flex gap-2">
              {singleProduct.images?.map((_: any, i: number) => (
                <button key={i} onClick={(e) => { e.stopPropagation(); goToSlide(i); }} className={`h-1.5 rounded-full transition-all ${i === currentIndex ? "w-8 bg-white" : "w-1.5 bg-white/40"}`} />
              ))}
            </div>
          </motion.div>
        )}
