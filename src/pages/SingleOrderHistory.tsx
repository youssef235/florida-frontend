      ...
      <div className="so-root">
        <div style={{ maxWidth: 900, margin: "0 auto", padding: "3rem 1.5rem" }}>
          ...
          <div style={{ marginBottom: "2rem", display: "flex", justifyContent: "space-between", alignItems: "flex-start", flexWrap: "wrap", gap: "1rem" }}>
            <div>
              <div className="so-title">{t("order.details_title")}</div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "2fr 1fr", gap: "1.5rem" }} className="so-grid-layout">
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "1.5rem" }}>
              ...
              <div className="so-card">
                <div className="so-section-title">{t("order.items_title")}</div>

                <table style={{ width: "100%", borderCollapse: "collapse" }}>
                  <thead>
                    <tr>
                      <th className="so-th" style={{ width: "45%" }}>{t("order.product")}</th>
                      <th className="so-th" style={{ textAlign: "center" }}>{t("order.qty")}</th>
                      <th className="so-th so-hide-mobile" style={{ textAlign: "right" }}>{t("order.price")}</th>
                      <th className="so-th" style={{ textAlign: "right" }}>{t("order.total")}</th>
                    </tr>
                  </thead>
                  ...
                  <tbody>
                    {order?.orderItems?.map((item: any) => (
                      <tr key={item._id}>
                        ...
                        <td className="so-td" style={{ textAlign: "center" }}>
                          {item.quantity}
                        </td>

                        <td className="so-td so-hide-mobile" style={{ textAlign: "right" }}>
                          {t("order.currency")} {Number(item.price).toFixed(0)}
                        </td>

                        <td className="so-td" style={{ textAlign: "right", fontFamily: "Syne, sans-serif", fontWeight: 700, color: "#0D0D0D" }}>
                          {t("order.currency")} {(Number(item.price) * item.quantity).toFixed(0)}
                        </td>
                      </tr>
                    ))}
                  </tbody>
              ...
              {order?.deliveryInfo && (
                <div className="so-card">
                  <div className="so-section-title">{t("order.address")}</div>
                  ...
                  <div style={{ fontSize: "0.9rem", color: "#444", lineHeight: 1.8 }}>
                    ...
                    <div style={{ marginTop: "0.25rem", color: "#777" }}>
            ...
            <div style={{ position: "sticky", top: "2rem", height: "fit-content" }}>
              ...
              <div className="so-dark-card">
                ...
                <div className="so-section-title">{t("order.summary")}</div>
